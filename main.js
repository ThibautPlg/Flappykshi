...
function preload() {
	...
	var assets = {
        spritesheet: {
            birdie: ['assets/rahkshi_med.png', 480, 240],
            clouds: ['assets/nuages2.png', 256, 128]
        },
        image: {
            finger: ['assets/column_low2.png'],
            fence: ['assets/sol.png']
        },
        audio: {
            flap: [''],
            score: [''],
            hurt: ['assets/hurt_low.wav']
        }
    };
...
function create() {
    ...
    credits = game.add.text(
        game.world.width / 2,
        10,
        'marksteve.com/dtmb @themarksteve\nFork by Piruk\n Sprites by Nuparu556\n#bionicle20ans',
        {
...
function reset() {
    ...
    scoreText.setText("Flappykshi");
    instText.setText("TOUCH TO FLAP\nTHE TURAHK");
    gameOverText.renderable = false;
    birdie.body.allowGravity = false;
    birdie.angle = 0;
    birdie.reset(20, game.world.height / 2);
    birdie.scale.setTo(.3, .3);
...
function spawnFinger(fingerY, flipped) {
    ...
    // Flip finger! *GASP*
    finger.scale.setTo(.5, flipped ? -.5 : .5);
    finger.body.offset.y = flipped ? -finger.body.height * .5 : 0;
...
function setGameOver() {
    gameOver = true;
	if (score == 0) {
		instText.setText("SERIOUSLY ? \n Not even funny, I know.");
	} else if (score<10) {
		instText.setText("TOUCH \nTO TRY AGAIN");
	} else if (score>20) {
		instText.setText("GET A LIFE, BRO");
	}
